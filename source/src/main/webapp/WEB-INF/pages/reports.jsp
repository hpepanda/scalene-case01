...
<div class="row">
	...
	<table>
		...
		<tbody>
			...
			<tr>
				<td><fmt:formatDate type="date" dateStyle="short" value="${reports[i].date}" /></td>				
				<td>
					${reports[i].name}
				</td>
				<td>
				...
				<td>
					<c:if test="${reports[i].status=='Pending'}">
						<a href="${pageContext.request.contextPath}/report/${reports[i].id}/edit">View / Edit</a>
					</c:if>
					<c:if test="${reports[i].status=='Approved'}">
						<a href="${pageContext.request.contextPath}/report/${reports[i].id}/view">View</a>
					</c:if>
				</td>
				<td>
				</td>
